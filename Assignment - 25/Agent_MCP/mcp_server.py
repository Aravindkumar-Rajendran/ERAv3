...
async def draw_rectangle(x1: int, y1: int, x2: int, y2: int) -> dict:
    ...
    try:
        ...
        # # Use coordinates to click on the Shapes button first
        # paint_window.click_input(coords=(300, 115))  # Click on Shapes button
        # time.sleep(0.5)
        
        # Click on the Rectangle tool specifically (4th shape)
        paint_window.click_input(coords=(678, 116))  # Rectangle tool
        time.sleep(1.0)
        
        # Get the canvas area
        canvas = paint_window.child_window(class_name='MSPaintView')
        
        # Get monitor info for coordinate adjustment
        # Get the Paint window position
        window_rect = win32gui.GetWindowRect(paint_window.handle)
        window_x, window_y = window_rect[0], window_rect[1]
        
        # Adjust coordinates relative to the Paint window
        adjusted_x1 = x1
        adjusted_y1 = y1
        adjusted_x2 = x2
        adjusted_y2 = y2
        ...
        # Draw rectangle - use direct canvas coordinates with proper timing
        # First, click to position at the start point
        canvas.click_input(coords=(adjusted_x1, adjusted_y1))
        time.sleep(0.5)
        
        # Now perform the drag operation directly on the canvas
        canvas.press_mouse_input(coords=(adjusted_x1, adjusted_y1))
        time.sleep(0.5)
        ...
        # As a backup, try clicking both corners in sequence
        canvas.click_input(coords=(adjusted_x1, adjusted_y1))
        time.sleep(0.3)
...
async def add_text_in_paint(text: str, x: int, y: int) -> dict:
    ...
    try:
        ...
        # paint_window.click_input(coords=(622, 85))
        # time.sleep(0.5)
...
async def open_paint() -> dict:
    ...
    try:
        ...
        win32gui.SetWindowPos(
            paint_window.handle,
            win32con.HWND_TOP,
            0, 0,  # Position it on primary monitor
